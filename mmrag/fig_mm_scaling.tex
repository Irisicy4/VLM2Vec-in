...
Downstream VQA accuracy of the frozen \textsc{Qwen2.5-VL-7B} reader (top-$5$) for
\textsc{GME-Qwen2-VL-2B} retrievers trained on nested InfoSeek query subsets at fixed compute
($500$ updates throughout). Across a $16\times$ range of \emph{available} training data the curve
is flat to within $2$ points, because at $500$ steps $\times$ batch $4$ only ${\sim}2{,}000$
queries are ever \emph{consumed} --- which is why the data-scaling result is
Table~\ref{tab:mm_consumption}, on a consumption axis, and this figure is only its control.
\emph{Both ladder series are the v1 listwise recipe} (anchored, deterministic top-$8$): filled
circles are fresh same-cluster retrains, open squares the pre-cluster-move recorded wave, shown
separately because absolute levels are not comparable across waves. The single \textsc{v3-pure}
point is the headline three-seed cell at $45{,}248$ available queries (all of
\texttt{pool\_train}, a different and smaller pool file than the ladder's). Dashed lines are the
fresh zero-shot and relevance-SFT reproductions. Whiskers: seed min--max.}
